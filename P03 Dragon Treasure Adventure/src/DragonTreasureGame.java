...
public class DragonTreasureGame {
    ...
    try {
      ...
      while (fileReader.hasNext()) {
        ...
        if (parts.length == 3) { // has a char at end, special type of room
          ...
          /*
           * Switch statements are like if and else ifs but only work on a simple condition like
           * someChar == 'A'
           */
          switch (parts[2]) {
            case "S":
  ...
  /**
   * Searches the roomList arraylist to find the room that has the same ID. You can assume that
  ...
  /**
   * Reads ands parses the information to set up the map by "connecting" rooms that are labeled to
  ...
  /**
   * Finds the room in the list that is the starting room. Create player object and places them
   * there.
   */
  ...
  /**
   * Finds the room in the list that is the treasure room. Create dragon object and places them
   * there.
   */
  ...
  /**
   * Handles player turn by prompting until getting a valid room to move into. Then moves them.
  ...
  /**
   * Updates the game state depending on the location of the player to either continue, lose if same
   * spot as dragon, win if in same spot as treasure room
   */
  ...
  /**
   * Continues the process of alternating between player and dragon turns until the game is over.
   */
  private static void gamePlayLoop() {
    ...
    while (currentGameState == GameState.CONTINUE) {
      ...
      dragonTurn();
      // System.out.println("Dragon Location:" +dragon.getCurrentRoom()); //Can comment back in to
      // see where they are
  ...
  /**
   * Depending on the end state of the game print out a different message on whether they won or
   * lost.
   * 
   * @author Michelle
   */
