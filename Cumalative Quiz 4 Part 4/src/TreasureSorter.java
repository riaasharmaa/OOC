...
/**
 * This class contains a single public method: sortTreasure(), along with several helper methods and
 * two tests. This public method uses the heap sort algorithm to sort treasure values in descending
 * order.
 * 
 * // TODO #1-9 below ask you to finish implementing the following methods (only make edits below
 * each of these numbered TODOs, as described for each): peekTreasure() - returns largest treasure
 * value from the root position of the max-heap percolateDown() - recursively enforces heap ordering
 * down from specified index heapify() - iteratively moves values from a randomly-ordered array into
 * heap order removeBestTreasure() - removes treasure value from root position in heap, and returns
 * new size sortTreasure() - computes and returns a sorted array of treasure values
 * 
...
public class TreasureSorter {
  ...
  /**
   * Returns treasure value from the root position in the max-heap (largest value) We suppose that
   * the provided heap array is NOT empty.
   * 
  ...
  private static void percolateDown(int index, Treasure[] heap, int size) {
    ...
    if (best.equals(leftChildValueOrNull)) {
      // TODO #2: implement percolate down through left subtree - see provided utility methods
      // below for some shortcuts!
      swap(heap, leftChildIndex, index);
      percolateDown(leftChildIndex, heap, size);
    } else if (best.equals(rightChildValueOrNull)) {
      // TODO #3: implement percolate down through right subtree - see provided utility methods
      // below for some shortcuts!
  ...
  /**
   * Reorganizes an array of Treasure values into a valid max heap. This heapify operation does NOT
   * operate in-place.
   * 
  ...
  /**
   * Removes treasure value from root position in heap, and then return heap's new size. Assume that
   * the provided array is NOT empty and is a valid max-heap.
   * 
   * @param heap a provided max-heap
  ...
  public static Treasure[] sortTreasure(Treasure[] values, int size) {
    ...
    while (heapSize > 0) {
      ...
      // TODO #9: remove the best value from the heap (see helper methods above!)
  ...
  /**
   * Returns the largest between the provided arguments. Null arguments are ignored (considered
  ...
  private static boolean testHeapify() {
    try {
      Treasure[] randomValues = new Treasure[] {new Treasure(3.1), new Treasure(64.0),
          new Treasure(1.6), new Treasure(98.6), new Treasure(32.0), new Treasure(9.8)};
      Treasure[] heapOfTreasure = new Treasure[] {new Treasure(98.6), new Treasure(64.0),
          new Treasure(9.8), new Treasure(3.1), new Treasure(32.0), new Treasure(1.6)};
  ...
  private static boolean testSort() {
    try {
      Treasure[] heapOfTreasure = new Treasure[] {new Treasure(98.6), new Treasure(32.0),
          new Treasure(64.0), new Treasure(1.6), new Treasure(3.1), new Treasure(9.8)};
      Treasure[] sortedTreasure = new Treasure[] {new Treasure(98.6), new Treasure(64.0),
          new Treasure(32.0), new Treasure(9.8), new Treasure(3.1), new Treasure(1.6)};

      System.out.println("Expected: " + Arrays.deepToString(sortedTreasure));
      System.out.println(
          "Actual:   " + Arrays.deepToString(sortTreasure(heapOfTreasure, heapOfTreasure.length)));
...
class Treasure implements Comparable<Treasure> {
  ...
  public int compareTo(Treasure o) {
    return Double.compare(this.value, ((Treasure) o).value);
  }

  @Override
  public boolean equals(Object o) {
    if (o instanceof Treasure)
      return ((Treasure) o).value == this.value;
    return false;
  }
}
